...
function DetailedBlog() {
  ...
  const handleMouseMove = (e) => {
    ...
    const rotateX = (y / rect.height) * 50; // Adjust 50 for sensitivity
    const rotateY = -(x / rect.width) * 50; // Adjust 50 for sensitivity
  
    // Calculate dynamic shadow
    const shadowX = rotateY * 5; // Adjust 5 for shadow depth
    const shadowY = rotateX * 5;
    const shadowBlur = 20; // Shadow blur intensity
    const shadowColor = `rgba(0, 0, 0, 0.2)`; // Shadow color
  
    container.style.transform = `rotateX(${rotateX}deg) rotateY(${rotateY}deg)`; // Add scale
    container.style.boxShadow = `${shadowX}px ${shadowY}px ${shadowBlur}px ${shadowColor}`;
  };
  
  const handleMouseLeave = () => {
    const container = containerRef.current;
    container.style.transform = "rotateX(0deg) rotateY(0deg) scale(1)"; // Reset scale
    container.style.boxShadow = "0px 4px 10px rgba(0, 0, 0, 0.1)"; // Reset shadow
  };
